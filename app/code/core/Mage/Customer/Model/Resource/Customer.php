...
class Mage_Customer_Model_Resource_Customer extends Mage_Eav_Model_Entity_Abstract
{
	    ...
	    public function loadByTelephone(Mage_Customer_Model_Customer $customer, $telephone, $testOnly = false)
    {
		...
		->addAttributeToFilter("telephone",array("eq"=>$telephone));
        $customerdata->getSelect()->limit(1);
		
 
		$emaillist=$customerdata->getData("email");
		
		if(count($emaillist)==0){
			return false;
		}
		
        $bind    = array('customer_email' => $emaillist[0]["email"]);
