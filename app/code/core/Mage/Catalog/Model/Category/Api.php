...
class Mage_Catalog_Model_Category_Api extends Mage_Catalog_Model_Api_Resource
{
    ...
    public function tree($parentId = null, $store = null)
    {
		...
		if($collection->getsize()==0){
			...
			return array("errorcode"=>0,"result"=>$result);
		}
	...
	public function result($em_featured){
		
		return $em_featured;
	}
	
		    public function activity($parentId = null, $store = null)
    {
        if (is_null($parentId) && !is_null($store)) {
            $parentId = Mage::app()->getStore($this->_getStoreId($store))->getRootCategoryId();
        } elseif (is_null($parentId)) {
		...
		$categories=$result['children'];
		
		 
 
 		  foreach($categories as $key=>$val){
			  
			  $cate=Mage::getModel("catalog/category")->load($val['category_id']);
				
		  return $cate->getImage();
	  $_productCollection = Mage::getResourceModel('catalog/product_collection')
	      ->joinField('category_id','catalog/category_product','category_id','product_id=entity_id',null,'left')
   ->addAttributeToFilter('category_id', array('eq' => $val['category_id']))
	 
	 ->load();
				
				$info=$_productCollection->getData();
				
				foreach($info as $k=>$v){
					 $productinfo= Mage::getModel('catalog/product')->load($v['entity_id']);
					$info[$k]['price']=$productinfo->getSpecialPrice();
				}
	 
	 
				$categories[$key]['product']=$info;
    ...
    public function test($productId, $store = null, $attributes = null, $identifierType = null)
    {
