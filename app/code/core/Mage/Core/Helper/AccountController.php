...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function indexAction()
    {
		...
		if(Mage::registry('trackstatus')==1){
		 $this->getLayout()->getBlock('footer')->setStatus(1);
		 Mage::registry('trackstatus',0);
		}
		
	//	print_r($this->getLayout()->getBlock('footer')->getStatus()); die();
	    ...
	    public function editMobileAction()
    {
        ...
        $block = $this->getLayout()->getBlock('customer_edit_mobile');
        if ($block) {
            $block->setRefererUrl($this->_getRefererUrl());
        }
        $data = $this->_getSession()->getCustomerFormData(true);
